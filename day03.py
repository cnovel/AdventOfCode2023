   ...
   def _process(self):
      ...
      for i in range(0, len(self.lines)):
         ...
         for n in numbers.finditer(l):
            ...
            y_s = i - 1
            y_e = i + 2
            nb_hits = len([(x,y) for x in range(x_s - 1, x_e + 1) for y in range(y_s, y_e) if (x,y) in symbol_pos])
            if nb_hits > 0:
               sum_a += int(n.group())
      ...
      for i in range(0, len(self.lines)):
         l = self.lines[i]
         for s in star_rg.finditer(l):
            s_pos = (s.span()[0], i)
            neighs = [n.value for n in number_blocks if n.next_to(s_pos)]
            if len(neighs) == 2:
